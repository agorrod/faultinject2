#define	FAULTINJECT_C_
#include "faultinject/internal/faultinject_internal.h"
...
const char	*fi_malloc_conf;
bool	opt_abort =
#ifdef FAULTINJECT_DEBUG
    true
#else
    false
#endif
    ;
bool	opt_junk =
#if (defined(FAULTINJECT_DEBUG) && defined(FAULTINJECT_FILL))
    true
#else
    false
#endif
...
#ifdef FAULTINJECT_THREADED_INIT
...
FAULTINJECT_SECTION(".CRT$XCU") FAULTINJECT_ATTR(used)
...
arenas_extend(unsigned ind)
{
	 ...
	 * failure.
	 */
	malloc_write("<faultinject>: Error initializing arena\n");
	if (opt_abort)
		abort();
...
stats_print_atexit(void)
{
	...
	fi_malloc_stats_print(NULL, NULL, NULL);
}
...
FAULTINJECT_ALWAYS_INLINE_C void
...
FAULTINJECT_ALWAYS_INLINE_C bool
malloc_init(void)
{
    ...
    char const **v_p, size_t *vlen_p)
{
	...
	*k_p = opts;
	...
	for (accept = false; accept == false;) {
		...
		case '\0':
			if (opts != *opts_p) {
				malloc_write("<faultinject>: Conf string ends "
				    "with key\n");
			}
			return (true);
		default:
			malloc_write("<faultinject>: Malformed conf string\n");
			return (true);
		}
	}
    ...
    size_t vlen)
{
	...
	malloc_printf("<faultinject>: %s: %.*s:%.*s\n", msg, (int)klen, k,
	    (int)vlen, v);
}
...
malloc_conf_init(void)
{
	 ...
	 * valgrind option remains in faultinject 3.x for compatibility reasons.
	 */
	if (config_valgrind) {
		...
		if (config_fill && opt_valgrind) {
			...
			opt_quarantine = FAULTINJECT_VALGRIND_QUARANTINE_DEFAULT;
			opt_redzone = true;
		}
	...
	for (i = 0; i < 3; i++) {
		...
		case 1: {
			...
			const char *linkname =
#  ifdef FAULTINJECT_PREFIX
			    "/etc/"FAULTINJECT_PREFIX"malloc.conf"
#  else
			    "/etc/malloc.conf"
#  endif
			    ;
			...
			const char *envname =
#ifdef FAULTINJECT_PREFIX
			    FAULTINJECT_CPREFIX"MALLOC_CONF"
#else
			    "MALLOC_CONF"
#endif
			    ;
...
malloc_init_hard(void)
{
	...
	if (opt_stats_print) {
		...
		if (atexit(stats_print_atexit) != 0) {
			malloc_write("<faultinject>: Error in atexit()\n");
			if (opt_abort)
				abort();
		}
	}
...
#if (!defined(FAULTINJECT_MUTEX_INIT_CB) && !defined(FAULTINJECT_ZONE) \
    && !defined(_WIN32))
	/* LinuxThreads's pthread_atfork() allocates. */
	if (pthread_atfork(faultinject_prefork, faultinject_postfork_parent,
	    faultinject_postfork_child) != 0) {
		malloc_write("<faultinject>: Error in pthread_atfork()\n");
		if (opt_abort)
			abort();
	}
#endif
	...
	if (narenas_auto > chunksize / sizeof(arena_t *)) {
		...
		malloc_printf("<faultinject>: Reducing narenas to limit (%d)\n",
		    narenas_auto);
	}
...
{
	void *ret;
	size_t usize FAULTINJECT_CC_SILENCE_INIT(0);
	...
	if (ret == NULL) {
		if (config_xmalloc && opt_xmalloc) {
			malloc_write("<faultinject>: Error in malloc(): "
			    "out of memory\n");
			abort();
		}
		set_errno(ENOMEM);
	...
	UTRACE(0, size, ret);
	FAULTINJECT_VALGRIND_MALLOC(ret != NULL, ret, usize, false);
	return (ret);
}
...
#ifdef FAULTINJECT_PROF
/*
 * Avoid any uncertainty as to how many backtrace frames to ignore in
 * PROF_ALLOC_PREP().
 */
FAULTINJECT_NOINLINE
#endif
static int
imemalign(void **memptr, size_t alignment, size_t size, size_t min_alignment)
{
	...
	if (malloc_init()) {
		...
		if (((alignment - 1) & alignment) != 0
			...
			if (config_xmalloc && opt_xmalloc) {
				malloc_write("<faultinject>: Error allocating "
...
label_oom:
	assert(result == NULL);
	if (config_xmalloc && opt_xmalloc) {
		malloc_write("<faultinject>: Error allocating aligned memory: "
		    "out of memory\n");
		abort();
	}
	ret = ENOMEM;
...
int
fi_posix_memalign(void **memptr, size_t alignment, size_t size)
{
	int ret = imemalign(memptr, alignment, size, sizeof(void *));
	FAULTINJECT_VALGRIND_MALLOC(ret == 0, *memptr, isalloc(*memptr,
	    config_prof), false);
	return (ret);
}

void *
fi_aligned_alloc(size_t alignment, size_t size)
{
	void *ret;
	int err;
	...
	FAULTINJECT_VALGRIND_MALLOC(err == 0, ret, isalloc(ret, config_prof),
	    false);
	return (ret);
}
...
void *
fi_calloc(size_t num, size_t size)
{
	void *ret;
	size_t num_size;
	size_t usize FAULTINJECT_CC_SILENCE_INIT(0);
...
label_return:
	if (ret == NULL) {
		if (config_xmalloc && opt_xmalloc) {
			malloc_write("<faultinject>: Error in calloc(): out of "
			    "memory\n");
			abort();
		}
		set_errno(ENOMEM);
	...
	FAULTINJECT_VALGRIND_MALLOC(ret != NULL, ret, usize, true);
	return (ret);
}
...
FAULTINJECT_INLINE_C void
ifree(void *ptr)
{
	size_t usize;
	UNUSED size_t rzsize FAULTINJECT_CC_SILENCE_INIT(0);
	...
	if (config_valgrind && opt_valgrind)
		rzsize = p2rz(ptr);
	iqalloc(ptr);
	FAULTINJECT_VALGRIND_FREE(ptr, rzsize);
}

void *
fi_realloc(void *ptr, size_t size)
{
	void *ret;
	size_t usize FAULTINJECT_CC_SILENCE_INIT(0);
	size_t old_usize = 0;
	UNUSED size_t old_rzsize FAULTINJECT_CC_SILENCE_INIT(0);
	...
	if (ret == NULL) {
		if (config_xmalloc && opt_xmalloc) {
			malloc_write("<faultinject>: Error in realloc(): "
			    "out of memory\n");
			abort();
		}
		set_errno(ENOMEM);
	...
	UTRACE(ptr, size, ret);
	FAULTINJECT_VALGRIND_REALLOC(ret, usize, ptr, old_usize, old_rzsize,
	    false);
	return (ret);
}
...
void *
fi_memalign(size_t alignment, size_t size)
{
	void *ret FAULTINJECT_CC_SILENCE_INIT(NULL);
	imemalign(&ret, alignment, size, 1);
	FAULTINJECT_VALGRIND_MALLOC(ret != NULL, ret, size, false);
	return (ret);
}
#endif
...
/*
 * is_malloc(fi_malloc) is some macro magic to detect if faultinject_defs.h has
 * #define fi_malloc malloc
 */
#define	malloc_is_malloc 1
#define	is_malloc_(a) malloc_is_ ## a
#define	is_malloc(a) is_malloc_(a)

#if ((is_malloc(fi_malloc) == 1) && defined(__GLIBC__) && !defined(__UCLIBC__))
/*
 ...
 * ignored.
 */
FAULTINJECT_EXPORT void (* __free_hook)(void *ptr) = fi_free;
FAULTINJECT_EXPORT void *(* __malloc_hook)(size_t size) = fi_malloc;
FAULTINJECT_EXPORT void *(* __realloc_hook)(void *ptr, size_t size) = fi_realloc;
FAULTINJECT_EXPORT void *(* __memalign_hook)(size_t alignment, size_t size) =
    fi_memalign;
#endif
...
void *
fi_mallocx(size_t size, int flags)
{
	void *p;
	size_t usize;
	...
	UTRACE(0, size, p);
	FAULTINJECT_VALGRIND_MALLOC(true, p, usize, zero);
	return (p);
label_oom:
	if (config_xmalloc && opt_xmalloc) {
		malloc_write("<faultinject>: Error in mallocx(): out of memory\n");
		abort();
	}
	UTRACE(0, size, 0);
	return (NULL);
...
void *
fi_rallocx(void *ptr, size_t size, int flags)
{
	void *p;
	...
	UTRACE(ptr, size, p);
	FAULTINJECT_VALGRIND_REALLOC(p, usize, ptr, old_usize, old_rzsize, zero);
	return (p);
label_oom:
	if (config_xmalloc && opt_xmalloc) {
		malloc_write("<faultinject>: Error in rallocx(): out of memory\n");
		abort();
	}
	UTRACE(ptr, size, 0);
	return (NULL);
...
FAULTINJECT_ALWAYS_INLINE_C size_t
...
size_t
fi_xallocx(void *ptr, size_t size, size_t extra, int flags)
{
	size_t usize, old_usize;
	UNUSED size_t old_rzsize FAULTINJECT_CC_SILENCE_INIT(0);
	size_t alignment = (ZU(1) << (flags & MALLOCX_LG_ALIGN_MASK)
	...
	FAULTINJECT_VALGRIND_REALLOC(ptr, usize, ptr, old_usize, old_rzsize, zero);
label_not_resized:
	UTRACE(ptr, size, ptr);
	return (usize);
}
...
size_t
fi_sallocx(const void *ptr, int flags)
{
	size_t usize;
...
size_t
fi_nallocx(size_t size, int flags)
{
	size_t usize;
	size_t alignment = (ZU(1) << (flags & MALLOCX_LG_ALIGN_MASK)
	    & (SIZE_T_MAX-1));
...
int
fi_mallctl(const char *name, void *oldp, size_t *oldlenp, void *newp,
    size_t newlen)
{
...
int
fi_mallctlnametomib(const char *name, size_t *mibp, size_t *miblenp)
{
...
int
fi_mallctlbymib(const size_t *mib, size_t miblen, void *oldp, size_t *oldlenp,
...
void
fi_malloc_stats_print(void (*write_cb)(void *, const char *), void *cbopaque,
    const char *opts)
{
...
size_t
fi_malloc_usable_size(FAULTINJECT_USABLE_SIZE_CONST void *ptr)
{
	size_t ret;
...
int
fi_allocm(void **ptr, size_t *rsize, size_t size, int flags)
{
	void *p;

	assert(ptr != NULL);

	p = fi_mallocx(size, flags);
	if (p == NULL)
...
int
fi_rallocm(void **ptr, size_t *rsize, size_t size, size_t extra, int flags)
{
	int ret;
	...
	if (no_move) {
		size_t usize = fi_xallocx(*ptr, size, extra, flags);
		ret = (usize >= size) ? ALLOCM_SUCCESS : ALLOCM_ERR_NOT_MOVED;
		if (rsize != NULL)
			*rsize = usize;
	} else {
		void *p = fi_rallocx(*ptr, size+extra, flags);
		if (p != NULL) {
			*ptr = p;
			ret = ALLOCM_SUCCESS;
		} else
...
int
fi_sallocm(const void *ptr, size_t *rsize, int flags)
{

	assert(rsize != NULL);
	*rsize = fi_sallocx(ptr, flags);
	return (ALLOCM_SUCCESS);
}

int
fi_dallocm(void *ptr, int flags)
{

	fi_dallocx(ptr, flags);
	return (ALLOCM_SUCCESS);
}

int
fi_nallocm(size_t *rsize, size_t size, int flags)
{
	size_t usize;

	usize = fi_nallocx(size, flags);
	if (usize == 0)
		return (ALLOCM_ERR_OOM);
	if (rsize != NULL)
		*rsize = usize;
 ...
 * partially initialized the allocator.  Ordinarily faultinject prevents
 * fork/malloc races via the following functions it registers during
 * initialization using pthread_atfork(), but of course that does no good if
 * the allocator isn't fully initialized at fork time.  The following library
 * constructor is a partial solution to this problem.  It may still possible to
 * trigger the deadlock described above, but doing so would involve forking via
 * a library constructor that runs before faultinject's runs.
 */
FAULTINJECT_ATTR(constructor)
static void
faultinject_constructor(void)
{

	malloc_init();
}

#ifndef FAULTINJECT_MUTEX_INIT_CB
void
faultinject_prefork(void)
#else
FAULTINJECT_EXPORT void
_malloc_prefork(void)
#endif
{
	unsigned i;

#ifdef FAULTINJECT_MUTEX_INIT_CB
...
#ifndef FAULTINJECT_MUTEX_INIT_CB
void
faultinject_postfork_parent(void)
#else
FAULTINJECT_EXPORT void
...
void
faultinject_postfork_child(void)
{
	unsigned i;
